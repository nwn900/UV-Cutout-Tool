...
if exist "%BUILD_PATH%\CMakeCache.txt" (
    findstr /C:"CMAKE_HOME_DIRECTORY:INTERNAL=%SCRIPT_DIR:\=/%" "%BUILD_PATH%\CMakeCache.txt" >nul
    if errorlevel 1 (
        echo.
        echo Existing CMake cache points at a different source folder.
        echo Clearing local build cache so this copy can configure cleanly...
        del /f /q "%BUILD_PATH%\CMakeCache.txt" >nul 2>nul
        if exist "%BUILD_PATH%\CMakeFiles" rmdir /s /q "%BUILD_PATH%\CMakeFiles"
    )
)

if exist "%BUILD_PATH%" (
    del /f /q "%BUILD_PATH%\.ninja_log.restat" >nul 2>nul
    del /f /q "%BUILD_PATH%\build.ninja.tmp*" >nul 2>nul
    del /f /q "%BUILD_PATH%\CMakeCache.txt.tmp*" >nul 2>nul
    del /f /q "%BUILD_PATH%\cmake_install.cmake.tmp*" >nul 2>nul
)

echo.
echo [2/4] Configuring CMake...
"%CMAKE_EXE%" -S "%SOURCE_PATH%" -B "%BUILD_PATH%" -G Ninja -DCMAKE_PREFIX_PATH="%QT_PREFIX%" -DCMAKE_BUILD_TYPE=Release
if errorlevel 1 (
